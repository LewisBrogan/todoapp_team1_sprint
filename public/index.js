...
function updateResponseArea(responseData) {
  ...
  if (responseData.length === 0) {
    ...
    responseHead.textContent = '2D00 List:';
    for (let i = 0; i < responseData.length; i++) {
      ...
      input.setAttribute('onclick', 'markedDone()');
...
function appHealth() {
  location.href = "https://2doot.eu-gb.cf.appdomain.cloud/health";
}
...
function markedDone() {
  for(var i = 0; i < 10; i++) {
    ...
    if(checkbox.checked == true) {
      ...
      newLi.textContent = listItem.innerHTML;
      newLi.style.color = "green";
      completedItems.appendChild(newLi);
    } else {
